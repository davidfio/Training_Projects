...
namespace FantomLib
{
    ...
    ///(*) Recommended since API 19(Android4.4). In the API Level before that, file information may not be get correctly according to the specification of the default folder or ContentProvider.
    ///(*) Sometimes it can not be get correctly depending on the authority (security) or the folder in which it is placed.
    ///(*) Depending on the saving condition of the image, it may not be possible to get width or height information (It becomes 0 when it can not be get).
    ...
    ///※API19(Android4.4)以降推奨。それより前の API Level ではデフォルトのフォルダやコンテンツプロバイダの仕様により、正しくファイル情報が取得できない可能性あり。
    ...
    public class GalleryPickController : MonoBehaviour
    {
        ...
        [Serializable] public class ResultInfoHandler : UnityEvent<ImageInfo> { }   //{path, uri, width, height}
