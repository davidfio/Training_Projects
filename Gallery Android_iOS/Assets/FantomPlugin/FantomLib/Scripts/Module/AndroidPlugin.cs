...
namespace FantomLib
{
    ...
    public static class AndroidPlugin
    {
        ...
        ///  サイズ（width, height）は保存状態によっては取得できない可能性がある（0 になる）。
        ...
        /// 
        /// 
        /// ストレージアクセスフレームワークでテキストファイルを選択し、ロードする。
        ///※API 19(Android 4.4)以上のみ。
        ...
        /// 
        /// 
        /// ストレージアクセスフレームワークでテキストファイルを選択し、ロードする（文字エンコード：UTF-8）。
        ///※API 19(Android 4.4)以上のみ。
        ///・成功したときは「resultCallbackMethod」に読み込んだテキストが返る。
        ///・失敗したときは以下のエラーメッセージが返る。
        /// CANCEL_LOAD_TEXT：選択せずに閉じられた
        ...
        /// 
        /// 
        /// ストレージアクセスフレームワークでディレクトリを指定し、保存する。
        ///※API 19(Android 4.4)以上のみ。
        ...
        /// UnsupportedEncodingException：サポートされてない文字エンコード
        /// </summary>
        /// <param name="fileName">Default file name (e.g. "NewDocument.txt". Does not include directory path)</param>
        ...
        /// 
        /// 
        /// ストレージアクセスフレームワークでディレクトリを指定し、保存する（文字エンコード：UTF-8）。
        ///※API 19(Android 4.4)以上のみ。
        ///・成功したときは「resultCallbackMethod」に保存したファイル名が返る。
        ///・失敗したときは以下のエラーメッセージが返る。
        /// CANCEL_SAVE_TEXT：選択せずに閉じられた
    ...
    //For content status (Mainly used for Media Scanner)
    //コンテンツ情報用（主に Media Scanner に利用）
    [Serializable]
    ...
    {
        //public string path = "";      //Based on ContentInfo
        //public string uri = "";       //Based on ContentInfo
        public int width;
        public int height;
