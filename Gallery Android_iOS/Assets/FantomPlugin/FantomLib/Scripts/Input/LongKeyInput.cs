...
namespace FantomLib
{
    ...
    public class LongKeyInput : MonoBehaviour
    {
        ...
        public float validTime = 2.0f;              //Time to recognize as long press (to recognize it as a long press with longer time)    //長押しとして認識する時間（これより長い時間で長押しとして認識する）
...
#region Properties and Local values Section
        ...
        bool isValid = false;       //For each frame validity           //フレーム毎判定用

        //Long press detection property (For each frame acquisition)    //長押検出プロパティ（フレーム毎取得用）
